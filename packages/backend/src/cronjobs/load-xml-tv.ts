...
async function deleteOld() {
    ...
    await getRepository(Programme).delete({
        end: Raw((alias) => `datetime(${alias}) < datetime("${new Date().toISOString()}")`),
    })
}
...
async function updateChannels(filePath: string) {
    ...
    await json?.tv?.channel?.forEach(async (channel: XmlChannel) => {
        const dbEntry = await getRepository(Channel).createQueryBuilder()
...
export async function loadXmlTv(xmlDir: string) {
    ...
    for (let index = 0; index < files.length; index ++) {
        ...
        let dbEntry = await getRepository(XmlFile).createQueryBuilder()
        ...
        if (dbEntry.md5sum !== fileMd5) {
            ...
            dbEntry.importDate = new Date(),
            await updateChannels(file.toString())
